...
function mirrorOutputChannels(outputChannels: Float32Array[], source: Float32Array): void {
 ...
 */
class WorkletRnnoise extends AudioWorkletProcessor {
  /**
  ...
  /**
   * The circular buffer uses a couple of indexes to track data segments. Input data from
   * the stream is copied to the circular buffer as it comes in, one `procNodeSampleRate`
   * sized sample at a time. inputBufferLength denotes the current length of all gathered
   * raw audio segments.
   */
  ...
  public constructor() {
    ...
    /**
     * The wasm module needs to be compiled to load synchronously as the audio worklet
     * `addModule()` initialization process does not wait for the resolution of promises
     ...
     * Because the audio worklet input array does not match the sample size required by
     * rnnoise two cases can occur
     * 1. There is not enough data in which case we buffer it.
     * 2. There is enough data but some residue remains after the call to `processAudioFrame`,
     * so its buffered for the next call.
  ...
  /**
   * Worklet interface process method. The inputs parameter contains PCM audio that is then
   * sent to rnnoise. Rnnoise only accepts PCM samples of 480 bytes whereas `process` handles
   * 128 sized samples, we take this into account using a circular buffer.
   *
   * @param {Float32Array[]} inputs - Array of inputs connected to the node, each of them with
   * their associated array of channels. Each channel is an array of 128 pcm samples.
   * @param {Float32Array[]} outputs - Array of outputs similar to the inputs parameter
   * structure, expected to be filled during the execution of `process`. By default each
   * channel is zero filled.
   * @returns {boolean} - Boolean value that returns whether or not the processor should remain
   * active. Returning false will terminate it.
   */
  public process(inputs: Float32Array[][], outputs: Float32Array[][]) {
    // We expect the incoming track to be mono, if a stereo track is passed only on of its
    // channels will get denoised and sent pack.
    // TODO Technically we can denoise both channel however this might require a new rnnoise
    // context, some more investigation is required.
    const [[inData]] = inputs;
    const [outputChannels] = outputs;
    const [outData] = outputChannels;
    ...
    // Determine how much denoised audio is available, if the start index of denoised samples
    // is smaller then denoisedBufferLength that means a rollover occured.
    ...
    // e.g. if the buffer size is 1024 samples but we only denoised 960 (this happens on the
    // first iteration) nothing happens, then on the next iteration 1920 samples will be
    // denoised so we send 1024 which leaves 896 for the next iteration and so on.
    if (unsentDenoisedDataLength >= outData.length) {
      ...
      mirrorOutputChannels(outputChannels, outData);
      this.denoisedBufferIndx += outData.length;
    }

    // When the end of the circular buffer has been reached, start from the beggining. By the
    // time the index starts over, the data from the begging is stale (has already been
    // processed) and can be safely overwritten.
    if (this.denoisedBufferIndx === this.circularBufferLength) {
      this.denoisedBufferIndx = 0;
    }

    // Because the circular buffer's length is the lcm of both input size and the processor's
    // sample size, by the time we reach the end with the input index the denoise length
    // index will be there as well.
