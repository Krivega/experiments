...
/**
 * Represents an adaptor for the rnnoise library compiled to webassembly. The class takes care
 * of webassembly memory management and exposes rnnoise functionality such as PCM audio
 * denoising and VAD (voice activity detection) scores.
 */
export default class RnnoiseProcessor {
  ...
  /**
   * The Float32Array index representing the start point in the wasm heap of the
   * wasmPcmInput buffer.
   */
  private readonly wasmPcmInputF32Index: number;

  /**
   * Reused scratch buffer: scale in one pass, then bulk-copy to WASM heap via
   * {@link Float32Array#set}.
   */
  private readonly scratchFrame = new Float32Array(RNNOISE_SAMPLE_LENGTH);

  /**
   * Constructor.
   *
   * @class
   * @param {Object} wasmInterface - WebAssembly module interface that exposes rnnoise
   * functionality.
   */
  public constructor(wasmInterface: TRnnoise) {
    // Considering that we deal with dynamic allocated memory employ exception safety
    // strong guarantee i.e. in case of exception there are no side effects.
    try {
   ...
   * The size of the array must be of exactly 480 samples, this constraint comes from
   * the rnnoise library.
   *
  ...
  /**
   * Process an audio frame, optionally denoising the input pcmFrame and returning the Voice
   * Activity Detection score for a raw Float32 PCM sample Array.
   * The size of the array must be of exactly 480 samples, this constraint comes from
   * the rnnoise library.
   *
   * @param {Float32Array} pcmFrame - Array containing 32 bit PCM samples. Parameter is also
   * used as output when {@code shouldDenoise} is true.
  ...
  public processAudioFrame(pcmFrame: Float32Array, shouldDenoise = false): number {
    // Convert 32 bit Float PCM samples to 16 bit Float PCM samples as that's what rnnoise
    // accepts as input: one scaling pass, then bulk copy into WASM heap.
    const scratch = this.scratchFrame;
    const heap = this.wasmInterface.HEAPF32;
    const heapOffset = this.wasmPcmInputF32Index;

    for (let i = 0; i < RNNOISE_SAMPLE_LENGTH; i++) {
      scratch[i] = pcmFrame[i] * SHIFT_16_BIT_NR;
    }

    heap.set(scratch, heapOffset);
    ...
    if (shouldDenoise) {
      // Convert back to 32 bit PCM: bulk copy from heap, then one scaling pass.
      pcmFrame.set(heap.subarray(heapOffset, heapOffset + RNNOISE_SAMPLE_LENGTH));

      const invScale = 1 / SHIFT_16_BIT_NR;
