...
const resolveSendAction = <T = unknown>(worker: Worker, type: string) => {
...
const resolveSendActionAndWaitConfirm = <P = unknown, R = unknown>(
  worker: Worker,
  type: string,
) => {
  return async (payload?: P, transfer?: Transferable[]): Promise<R> => {
...
type THandlerAction<P = unknown> = (
  payload: P,
) => Promise<{ payload?: P; transfer?: Transferable[] }>;

const resolveOnReceiveActionAndWaitConfirm = <P = unknown>(worker: Worker, type: string) => {
