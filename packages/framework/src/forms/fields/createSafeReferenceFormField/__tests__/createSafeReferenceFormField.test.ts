...
import { createSafeReferenceFormField } from '..';

import type { TInitialState } from '@experiments/mst-tools';

describe('createSafeReferenceFormField', () => {
  describe('базовая функциональность', () => {
    ...
    const TestModel = types
      .model({
        selectedItem: createSafeReferenceFormField(ItemModel, {
    ...
    it('должен инициализировать поле с начальными значениями', () => {
      ...
      expect(selectedItem.error).toBe(undefined);
      expect(selectedItem.isClientError).toBe(false);
      expect(selectedItem.isServerError).toBe(false);
      expect(selectedItem.disabled).toBe(false);
      expect(selectedItem.getField()).toHaveProperty('getValue');
      expect(selectedItem.getField()).toHaveProperty('getError');
      expect(selectedItem.getField()).toHaveProperty('hasValid');
    ...
    describe('управление состоянием ошибки', () => {
      it('должен получать ошибку поля через getField.getError()', () => {
        selectedItem.setClientError('new error');

        expect(selectedItem.error?.value).toBe('new error');
        expect(selectedItem.getField().getError()).toBe('new error');
      });

      it('должен устанавливать клиентскую ошибку через setClientError', () => {
        selectedItem.setClientError('new error');

        expect(selectedItem.error).toEqual({ value: 'new error', type: 'client' });
        expect(selectedItem.isClientError).toBe(true);
        expect(selectedItem.isServerError).toBe(false);
      });

      it('должен устанавливать серверную ошибку через setServerError', () => {
        selectedItem.setServerError('new error');

        expect(selectedItem.error).toEqual({ value: 'new error', type: 'server' });
        expect(selectedItem.isClientError).toBe(false);
        expect(selectedItem.isServerError).toBe(true);
      });

      it('должен сбрасывать клиентскую ошибку через resetError', () => {
        selectedItem.setClientError('new error');
        selectedItem.resetError();

        expect(selectedItem.error).toBeUndefined();
        expect(selectedItem.isClientError).toBe(false);
        expect(selectedItem.isServerError).toBe(false);
      });
    ...
    describe('управление состоянием валидности', () => {
      it('должен проверять валидность поля через getField.hasValid()', () => {
        expect(selectedItem.isClientError).toBe(false);
        expect(selectedItem.isServerError).toBe(false);
        expect(selectedItem.getField().hasValid()).toBe(true);

        selectedItem.setClientError('new error');

        expect(selectedItem.isClientError).toBe(true);
        expect(selectedItem.isServerError).toBe(false);
        expect(selectedItem.getField().hasValid()).toBe(false);
