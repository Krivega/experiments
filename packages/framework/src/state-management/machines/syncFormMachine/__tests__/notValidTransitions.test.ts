...
describe('syncActor: transitions from NOT_VALID', () => {
  ...
  it('should initially be NOT_VALID', () => {
  ...
  it('should not transition to SYNC_IN_PROGRESS', () => {
    ...
    expect(syncActor.getSnapshot().matches(NOT_VALID)).toBe(true);
  ...
  it('should transition from NOT_VALID to SYNCED', () => {
    syncActor.send({ type: EVENT_CANCEL });

    expect(syncActor.getSnapshot().matches(SYNCED)).toBe(true);
  });

  it('should transition from NOT_VALID to NOT_SAVED', () => {
    syncActor.send({ type: EVENT_CHANGE });
