...
> {
  ...
  public run() {
    return AbstractExecutableAction.resolveAsyncActionNoParams((): unknown => {
      if (this.isServerApiErrorMock) {
        throw this.serverApiError;
      }
...
describe('AbstractExecutableAction', () => {
  ...
  describe('Метод cancel', () => {
    it('должен вызывать метод cancel', () => {
      ...
      // Проверяем, что cancel был вызван (abort функция из resolveAsyncActionNoParams)
      // @ts-expect-error
      expect(action.abortPromise).toBeDefined();
    });
  });
});
