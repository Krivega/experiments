...
import resolveSetter from '../index';
...
describe('resolveSetter', () => {
  ...
  const ChildModel = types.model({ value: types.number }).actions((self) => {
  ...
  const Model = types
    ...
    .actions((self) => {
      ...
      return {
        setValueChildModel: resolveSelfSetter('valueChildModel'),
        setValueNumber: resolveSelfSetter('valueNumber'),
        setValueString: resolveSelfSetter('valueString'),
        setValueDate: resolveSelfSetter('valueDate'),
        setValueBoolean: resolveSelfSetter('valueBoolean'),
        setValueNull: resolveSelfSetter('valueNull'),
        setValueUndefined: resolveSelfSetter('valueUndefined'),
        setValueMapString: resolveSelfSetter('valueMapString'),
        setValueArrayString: resolveSelfSetter('valueArrayString'),
        setValueReference: resolveSelfSetter('valueReference'),
        setValueOptionalString: resolveSelfSetter('valueOptionalString'),
        setValueSafeReference: resolveSelfSetter('valueSafeReference'),
        setValueMayBeNumber: resolveSelfSetter('valueMayBeNumber'),
        setValueEnumeration: resolveSelfSetter('valueEnumeration'),
      };
    })
    .volatile(() => {
      return {
  ...
  describe('поведение с undefined значениями', () => {
    ...
    it('resolveSetter игнорирует undefined для optional полей', () => {
      const OptionalModel = types
        ...
        .actions((self) => {
          const resolveSelfSetter = resolveSetter(self);
          const setOptionalField = resolveSelfSetter('optionalField');
      ...
      // Значение по умолчанию, не undefined, но поле перезаписывается
      expect(instance.optionalField).toBe(0);
    });

    it('resolveSetter устанавливает undefined для maybe полей', () => {
      const MaybeModel = types
        ...
        .actions((self) => {
          const resolveSelfSetter = resolveSetter(self);
          const setMaybeField = resolveSelfSetter('maybeField');
      ...
      instance.setMaybeField(2);

      expect(instance.maybeField).toBe(2);

      instance.setMaybeField(undefined);

      expect(instance.maybeField).toBe(undefined);
    });

    it('resolveSetter выводит warning при undefined значении', () => {
      const MaybeModel = types
        .model({
          maybeField: types.maybe(types.number),
        })
  ...
  describe('поведение с массивами', () => {
    it('resolveSetter выводит warning при значении-массиве', () => {
      const ArrayModel = types
        .model({
          valueArrayString: types.array(types.string),
        })
        .actions((self) => {
          const resolveSelfSetter = resolveSetter(self);
          const setValueArrayString = resolveSelfSetter('valueArrayString');

          return { setValueArrayString };
        });

      const instance = ArrayModel.create({ valueArrayString: ['a'] });

      const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {
        return undefined;
      });

      instance.setValueArrayString(['b']);

      expect(warnSpy).toHaveBeenCalledTimes(1);

      const warningMessage = warnSpy.mock.calls[0]?.[0] as string;

      expect(warningMessage).toEqual(expect.stringContaining('является массивом'));
      expect(warningMessage).toEqual(expect.stringContaining('resolveReplacer'));
      expect(instance.valueArrayString).toEqual(['b']);
